...
import { simulateTrace } from '@/evaltrace.js';
...
// Generic evaluation-trace panel: every "basics & quirks" tab is the same
// player over a different scenario list. DOM ids are scoped by the root ref,
// so several instances never collide.
const props = defineProps({
  scenarios: { type: Array, required: true },
  lang: { type: String, default: 'js' }, // highlight.js language
  label: { type: String, required: true }, // code-box header
  noteKey: { type: String, required: true }, // i18n key of the bottom hint
});
...
onMounted(() => {
  ...
  player.setSpeed(+$('tp-speed').value);

  const setLabel = wirePlayerButtons({ playBtn: $('tp-play'), stepBtn: $('tp-step'), resetBtn: $('tp-reset'), player });

  const regenerate = () => {
    const sc = props.scenarios.find((s) => s.id === select.value);
    codeEl.innerHTML = highlight(sc.code, props.lang);
    lines = [];
    player.load([...simulateTrace(sc.ops)]);
    setLabel(false);
    render();
  };
  select.onchange = regenerate;
  $('tp-speed').oninput = () => player.setSpeed(+$('tp-speed').value);
  ...
  <section ref="root" class="panel">
    <div class="controls">
      <label><span data-i18n="lb.scenario">{{ tt('lb.scenario') }}</span>
        <select id="tp-scenario"></select>
      </label>
      <label><span data-i18n="ctrl.speed">{{ tt('ctrl.speed') }}</span>
        <input id="tp-speed" type="range" min="1" max="100" value="25" />
      </label>
      <button id="tp-play" class="primary">▶ Play</button>
      <button id="tp-step" data-i18n="btn.step">{{ tt('btn.step') }}</button>
      <button id="tp-reset" data-i18n="btn.reset">{{ tt('btn.reset') }}</button>
    </div>
    <div class="code-box">
      <div class="code-head"><span>{{ label }}</span></div>
      <pre><code id="tp-code"></code></pre>
    </div>
    <h3 class="rec-h" data-i18n="lb.journal">{{ tt('lb.journal') }}</h3>
    <div id="tp-log" class="el-console"></div>
    <p class="hint">{{ tt(noteKey) }}</p>
    <div class="status-row"><span id="tp-status" class="status"></span></div>
  </section>
</template>
