...
import { RUSTBASICS_SCENARIOS, rustBasicsScenarioById, simulate } from '@/rustbasics.js';
...
onMounted(() => {
  ...
  const select = $('rs-scenario');
  const logEl = $('rs-log');
  const status = $('rs-status');
  const codeEl = $('rs-code');
  for (const s of RUSTBASICS_SCENARIOS) {
  ...
  player.setSpeed(+$('rs-speed').value);

  const setLabel = wirePlayerButtons({ playBtn: $('rs-play'), stepBtn: $('rs-step'), resetBtn: $('rs-reset'), player });

  const regenerate = () => {
    const sc = rustBasicsScenarioById(select.value);
    codeEl.innerHTML = highlight(sc.code, 'rust');
    lines = [];
  ...
  $('rs-speed').oninput = () => player.setSpeed(+$('rs-speed').value);
  ...
  <section ref="root" class="panel">
    <div class="controls">
      ...
      <label><span data-i18n="ctrl.speed">{{ tt('ctrl.speed') }}</span>
        <input id="rs-speed" type="range" min="1" max="100" value="25" />
      </label>
      <button id="rs-play" class="primary">▶ Play</button>
      <button id="rs-step" data-i18n="btn.step">{{ tt('btn.step') }}</button>
      <button id="rs-reset" data-i18n="btn.reset">{{ tt('btn.reset') }}</button>
    </div>
    <div class="code-box">
      <div class="code-head"><span>Rust</span></div>
      <pre><code id="rs-code"></code></pre>
    </div>
    <h3 class="rec-h" data-i18n="lb.journal">{{ tt('lb.journal') }}</h3>
    <div id="rs-log" class="el-console"></div>
    <p class="hint" data-i18n="rsb.note">{{ tt('rsb.note') }}</p>
    <div class="status-row"><span id="rs-status" class="status"></span></div>
  </section>
</template>
